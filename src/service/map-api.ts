...
export class MapApi {
  public getMapItem(category: Category) {
  ...
  public sendSearchRequest(query: string, mapItems: Ref<MapItem[]>) {
    // mapItems: Ref<MapItem[]>

    const encodedQuery = encodeURI(query);

    return api.get(`/v1/search/local.json?query=${encodedQuery}&display=10&start=1&sort=random`, {
      headers: {
